    ...
    for (const [absPath, cached] of FILE_AST_CACHE) {
      ...
      traverse(ast, {
        ImportDeclaration(importPath) {
          ...
          // Some Babel path shapes don't expose parentPath.parentPath here, so
          // resolve a safe traversal root first.
          const traversalRoot =
            importPath.findParent((p) => p.isFile()) ||
            importPath.findParent((p) => p.isProgram()) ||
